...
# retrieved top-1 relevant vs. distracting documents.
...
# Usage:
#     python scripts/step8_train_probe_retrieved.py --model gemma2b
#     python scripts/step8_train_probe.py --model llama8b
#     python scripts/step8_train_probe_retrieved.py --model all

# Output:
#     results/probe/{model_key}_probe_results.json
#     results/probe_retrieved/{model_key}_auroc_by_layer.png
# """
...
# This script intentionally uses retrieved-doc hidden states and writes to
# probe_retrieved so the old gold-vs-distractor probe results are preserved.
...
#         print(f"  [skip] Retrieved hidden states not found for {model_key}. Run step6_extract_hidden_states_retrieved.py first.")
#         return
...
"""
Step 8R: Train Retrieved-Doc Probes at Every Layer (LR / MLP / NearestCentroid)

For each layer, train a probe on the hidden states to distinguish
retrieved top-1 relevant vs. distracting documents. Three classifier families are run:
...
Usage:
    python scripts/step8_train_probe_retrieved.py --model gemma2b
    python scripts/step8_train_probe_retrieved.py --model gemma9b
    python scripts/step8_train_probe_retrieved.py --model qwen3_4b
    python scripts/step8_train_probe_retrieved.py --model all

    # Run a single classifier only
    python scripts/step8_train_probe_retrieved.py --model gemma2b --classifier mlp

Output (per model):
    results/probe_retrieved/{model_key}_probe_results_lr.json
    results/probe_retrieved/{model_key}_probe_results_mlp.json
    results/probe_retrieved/{model_key}_probe_results_centroid.json
    results/probe_retrieved/{model_key}_probe_results_mass_mean.json
    results/probe_retrieved/{model_key}_auroc_by_layer.png   # all classifiers in one figure
...
def process_model(model_key: str, classifiers_to_run: list[str]):
    ...
    ax.set_title(f"{model_key} - Retrieved-Doc Probe AUROC by Layer (combined subset)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
